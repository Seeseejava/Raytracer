...
class noise_texture : public texture {
 public:
  ...
  virtual vec3 value(double u, double v, const vec3& p) const {
    //return vec3(1, 1, 1) * 0.5 *(1.0 + noise.noise(scale *p));  //这里的scale实际上是改变频率，scale越大频率越高
    ////柏林插值的输出结果有可能是负数, 这些负数在伽马校正时经过开平方跟`sqrt()`会变成NaN。我们将输出结果映射到0与1之间
    return vec3(1, 1, 1) * 0.5 * (1 + sin(scale * p.z() + 10 * noise.turb(p)));
    /*基本思路是让颜色与sine函数的值成比例,并使用扰动函数去调整相位(平移了sin(x) 中的x),
    使得带状条纹起伏波荡。修正我们直接使用扰动turb或者噪声noise给颜色赋值的方法，
    我们会得到一个类似大理石的纹理*/
  }
